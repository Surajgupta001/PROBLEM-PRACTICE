/*
Cinema Seat Allocation - [Leetcode - 1386(Medium)]
------------------------------------------------------
...
int maxNumberOfFamilies(int n, vector<vector<int>>& reservedSeats) {
    unordered_map<int, unordered_set<int>> map; // row -> set of reserved seats
    ...
    int result = (n - map.size()) * 2; // rows with no reserved seats can accommodate 2 groups

    for(auto &[row, bookedSeats] : map){
        // Lambda Function to check if a seat is available
...
int maxNumberOfFamilies(int n, vector<vector<int>>& reservedSeats) {
    unordered_map<int, int> map; // row -> bitmask of reserved seats

    for(auto &reservedSeat : reservedSeats){
        int row  = reservedSeat[0];
        int seat = reservedSeat[1];

        map[row] |= (1 << (seat)); // Set the bit corresponding to the reserved seat
    }
...
int main() {
    ...
    cout << maxNumberOfFamilies(n, reservedSeats) << endl; // Output: 4
    return 0;
}