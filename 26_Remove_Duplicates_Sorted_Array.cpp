...
int removeDuplicates(vector<int>& nums) {
    vector<int> temp;
    temp.push_back(nums[0]); // First element is always unique
    for(int i=1; i<nums.size(); i++){
        if(nums[i] != nums[i-1]){
            temp.push_back(nums[i]);
        }
    }
    for(int i=0; i<temp.size(); i++){ // Copying elements back to original array
        nums[i] = temp[i];
    }
    return temp.size();         
}
...
int removeDuplicates(vector<int>& nums) {
    int i = 0; // Pointer for the position of the last unique element
    int j = 1; // Pointer for traversing the array
    while(j < nums.size()){
        if(nums[j] != nums[i]){
            i++;
            nums[i] = nums[j];
