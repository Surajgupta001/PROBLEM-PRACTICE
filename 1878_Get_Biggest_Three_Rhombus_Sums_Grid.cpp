/*
Get Biggest Three Rhombus Sums in a Grid - [Leetcode - 1878(Medium)]
-----------------------------------------------------------------------
...
vector<int> getBiggestThree(vector<vector<int>>& grid) {
    ...
    for (int row = 0; row < m; row++) {
        for (int col = 0; col < n; col++) {
            ...
            addToSet(grid[row][col]);   //every cell is valid for rohmbus of side = 0
            
            for (int side = 1; row-side >= 0 && row+side < m && col-side >= 0 && col+side < n; side++) {
                int sum = 0;
                for (int k = 0; k < side; k++) {
                    sum += grid[row - side + k][col + k];   // top to right corner
                    sum += grid[row + k][col + side - k];   // right to bottom corner
                    sum += grid[row + side - k][col - k];   // bottom to left corner
                    sum += grid[row - k][col - side + k];   // left to top corner
                }
                addToSet(sum);
            }
        }
    }
    return vector<int>(rbegin(sets), rend(sets));
}
...
vector<int> getBiggestThree(vector<vector<int>>& grid) {
    ...
    return vector<int>(sets.rbegin(), sets.rend());
}
