...
int xorAfterQueries(vector<int>& nums, vector<vector<int>>& queries) {
    ...
    for(auto& entry : smallKMap) {
        int K = entry.first;
        vector<vector<int>>& allQueries = entry.second;
...
int main() {
    ...
    cout << xorAfterQueries(nums1, queries1) << endl; // Output: 4

    vector<int> nums2 = {2, 3, 1, 5, 4};
    vector<vector<int>> queries2 = {{1, 4, 2, 3}, {0, 2, 1, 2}};
    cout << xorAfterQueries(nums2, queries2) << endl; // Output: 31
